...
function addTransactionToBatch(tx) {
  ...
  return db.oneOrNone(sql, [tx.cryptoCode]).then(batch => {
    if (_.isNil(batch)) {
      return db.tx(t => {
        const newBatchId = uuid.v4()
        const q1 = t.none(
          `INSERT INTO transaction_batches (id, crypto_code) VALUES ($1, $2)`,
          [newBatchId, tx.cryptoCode],
        )
...
function confirmSentBatch(batch, tx) {
  return db.tx(t => {
    const q1 = t.none(
      `UPDATE transaction_batches SET status='sent', error_message=NULL WHERE id=$1`,
      [batch.id],
    )
    const q2 = t.none(
      `UPDATE cash_in_txs SET tx_hash=$1, fee=$2, send=true, send_confirmed=true, send_time=now(), send_pending=false, error=NULL, error_code=NULL WHERE batch_id=$3`,
      [tx.txid, tx.fee.toString(), batch.id],
    )
